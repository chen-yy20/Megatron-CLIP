...
def get_batch(data_iterator):
    ...
    if data_iterator is not None:
        ...
        keys = ['text']
        data['text'] = combine_data[1].to(dtype=torch.int32)
...
def loss_func(output):
    ...
    text_img_split = len(combine_output) // 2
    text_output = combine_output[:text_img_split]
    image_output =  combine_output[text_img_split:]
...
def forward_step(data_iterator, model):
    ...
    # FIXME: a better way to identify sub-models
    if model.config.down_or_up == "down":
        modality_key = "image"
    else:
        modality_key = "text"
    inputs = input_pairs[modality_key]
    loss = model(inputs)
